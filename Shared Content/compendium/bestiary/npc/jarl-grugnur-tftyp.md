---
cssclass: json5e-monster
tags:
- compendium/src/tftyp
- monster/size/huge
- monster/type/giant
- monster/environment/mountain
aliases: ["Jarl Grugnur"]
statblock: true
"name": "Jarl Grugnur"
"size": "Huge"
"type": "giant"
"alignment": "Neutral Good or Neutral Evil"
"ac": !!int "18"
"hp": !!int "200"
"hit_dice": "16d12 + 96"
"stats":
- !!int "27"
- !!int "10"
- !!int "22"
- !!int "12"
- !!int "16"
- !!int "16"
"speed": "walk 40 ft."
"saves":
  "Charisma": !!int "7"
  "Wisdom": !!int "7"
  "Constitution": !!int "10"
"skillsaves":
  "Insight": !!int "7"
  "Perception": !!int "7"
"damage_immunities": "cold"
"senses": "passive Perception 17"
"languages": "Common, Giant"
"cr": "9"
"actions":
- !!dev.ebullient.json5e.qute.Trait
  "desc": "Grugnur makes two longsword attacks."
  "name": "Multiattack"
- !!dev.ebullient.json5e.qute.Trait
  "desc": "Melee Weapon Attack: +14 to hit, reach 10 ft., one target. Hit: 23 (3d8\
    \ + 10) slashing damage, or 26 (3d10 + 10) slashing damage if used with both hands."
  "name": "Longsword +2"
- !!dev.ebullient.json5e.qute.Trait
  "desc": "Ranged Weapon Attack: +12 to hit, range 60/240 ft., one target. Hit:\
    \ 30 (4d10 + 8) bludgeoning damage."
  "name": "Rock"
"source":
- "TftYP"
name: Jarl Grugnur
image: "[[Jarl Grugnur.png]]"
---

# Jarl Grugnur

```statblock
"name": "Jarl Grugnur"
"size": "Huge"
"type": "giant"
"alignment": "Neutral Good or Neutral Evil"
"ac": !!int "18"
"hp": !!int "200"
"hit_dice": "16d12 + 96"
"stats":
- !!int "27"
- !!int "10"
- !!int "22"
- !!int "12"
- !!int "16"
- !!int "16"
"speed": "walk 40 ft."
"saves":
  "Charisma": !!int "7"
  "Wisdom": !!int "7"
  "Constitution": !!int "10"
"skillsaves":
  "Insight": !!int "7"
  "Perception": !!int "7"
"damage_immunities": "cold"
"senses": "passive Perception 17"
"languages": "Common, Giant"
"cr": "9"
"actions":
- !!dev.ebullient.json5e.qute.Trait
  "desc": "Grugnur makes two longsword attacks."
  "name": "Multiattack"
- !!dev.ebullient.json5e.qute.Trait
  "desc": "Melee Weapon Attack: +14 to hit, reach 10 ft., one target. Hit: 23 (3d8\
    \ + 10) slashing damage, or 26 (3d10 + 10) slashing damage if used with both hands."
  "name": "Longsword +2"
- !!dev.ebullient.json5e.qute.Trait
  "desc": "Ranged Weapon Attack: +12 to hit, range 60/240 ft., one target. Hit:\
    \ 30 (4d10 + 8) bludgeoning damage."
  "name": "Rock"
"source":
- "TftYP"
"image": "[[Jarl Grugnur.png]]"
```
^statblock

*Source: Tales from the Yawning Portal p. 189*

## Environment

mountain